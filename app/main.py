...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        # Integrated FFmpeg renderer
        self.render_worker: RenderWorker | None = None
        self.imported_audio_path: Path | None = None
    ...
    def create_voice_page(self):
        ...
        self.edit_subtitle_button = QPushButton(
            "ویرایش زیرنویس"
        )
        self.edit_subtitle_button.setObjectName(
            "editSubtitleButton"
        )
        self.edit_subtitle_button.setToolTip(
            "باز کردن فایل زیرنویس تولیدشده برای ویرایش"
        )
        self.edit_subtitle_button.clicked.connect(
            self.open_subtitle_editor
        )
        self.edit_subtitle_button.setEnabled(False)

        form.addWidget(
            self.edit_subtitle_button,
            4,
            0,
            1,
            3,
        ...
        self.refresh_subtitle_editor_button()
    ...
    def create_image_page(self):
        ...
        image_button = QPushButton("انتخاب تصویر یا قالب HTML")
        ...
        self.horizontal_radio = QRadioButton(
        ...
        self.format_group.addButton(self.horizontal_radio, 1)
        self.format_group.addButton(self.square_radio, 2)
        ...
        settings_layout.addWidget(self.horizontal_radio)
        settings_layout.addWidget(self.square_radio)
        settings_layout.addSpacing(8)
        ...
        preview_title = QLabel("پیش‌نمایش تصویر یا قالب HTML")
        preview_title.setObjectName("cardTitle")

        self.image_preview = QLabel(
            "تصویر یا قالب HTML انتخاب‌شده\nدر این قسمت نمایش داده می‌شود"
        )
    ...
    def create_output_page(self):
        ...
        self.preview_button.clicked.connect(
            lambda checked=False: self.start_video_render(
                preview=True
            )
        )
        ...
        self.cancel_render_button = QPushButton("لغو رندر")
        self.cancel_render_button.setEnabled(False)
        self.cancel_render_button.clicked.connect(
            self.cancel_video_render
        )

        output_buttons = QHBoxLayout()
        output_buttons.addWidget(self.preview_button)
        output_buttons.addWidget(self.render_button)
        output_buttons.addWidget(self.cancel_render_button)

        render_settings_title = QLabel("تنظیمات رندر")
        render_settings_title.setObjectName("cardTitle")

        render_settings = QGridLayout()
        render_settings.setHorizontalSpacing(12)
        render_settings.setVerticalSpacing(10)

        self.render_fps_spin = QSpinBox()
        self.render_fps_spin.setRange(1, 60)
        self.render_fps_spin.setValue(30)
        self.render_fps_spin.setSuffix(" fps")

        self.render_crf_spin = QSpinBox()
        self.render_crf_spin.setRange(16, 35)
        self.render_crf_spin.setValue(23)

        self.render_subtitle_size_spin = QSpinBox()
        self.render_subtitle_size_spin.setRange(16, 90)
        self.render_subtitle_size_spin.setValue(38)

        self.render_subtitle_margin_spin = QSpinBox()
        self.render_subtitle_margin_spin.setRange(0, 600)
        self.render_subtitle_margin_spin.setValue(150)
        self.render_subtitle_margin_spin.setSuffix(" px")

        self.render_audio_button = QPushButton(
            "انتخاب صدای ضبط‌شده یا آماده"
        )
        self.render_audio_button.clicked.connect(
            self.choose_render_audio
        )

        self.render_audio_label = QLabel(
            "صدا: فایل تولیدشده توسط برنامه"
        )
        self.render_audio_label.setWordWrap(True)

        render_settings.addWidget(QLabel("نرخ فریم:"), 0, 0)
        render_settings.addWidget(self.render_fps_spin, 0, 1)
        render_settings.addWidget(QLabel("کیفیت CRF:"), 0, 2)
        render_settings.addWidget(self.render_crf_spin, 0, 3)

        render_settings.addWidget(
            QLabel("اندازه زیرنویس:"),
            1,
            0,
        )
        render_settings.addWidget(
            self.render_subtitle_size_spin,
            1,
            1,
        )
        render_settings.addWidget(
            QLabel("فاصله زیرنویس از پایین:"),
            1,
            2,
        )
        render_settings.addWidget(
            self.render_subtitle_margin_spin,
            1,
            3,
        )

        render_settings.addWidget(
            self.render_audio_button,
            2,
            0,
            1,
            2,
        ...
        summary_layout.addWidget(render_settings_title)
        summary_layout.addLayout(render_settings)
        summary_layout.addSpacing(8)
    ...
    def choose_image(self):
        file_name, _ = QFileDialog.getOpenFileName(
            self,
            "انتخاب تصویر یا قالب HTML",
            str(PROJECT_ROOT / "input"),
            (
                "تصویر یا قالب HTML "
                "(*.png *.jpg *.jpeg *.webp *.bmp *.html *.htm);;"
                "فایل‌های تصویری "
                "(*.png *.jpg *.jpeg *.webp *.bmp);;"
                "قالب‌های HTML (*.html *.htm);;"
                "همه فایل‌ها (*.*)"
            ),
        )
        ...
        if suffix in SUPPORTED_IMAGE_EXTENSIONS:
            ...
            if pixmap.isNull():
                QMessageBox.warning(
                    self,
                    "تصویر نامعتبر",
                    "امکان باز کردن تصویر انتخاب‌شده وجود ندارد."
                    f"\n\n{file_path}",
                )
                return
            ...
            self.status_label.setText(
                "وضعیت: تصویر پس‌زمینه انتخاب شد"
            )

        elif suffix in {".html", ".htm"}:
            # HTML انتخاب‌شده منبع تصویری رندر خواهد بود.
            self.current_text_file = file_path

            # پاک‌کردن تصویر قبلی تا HTML در اولویت رندر قرار بگیرد.
            self.current_image_file = None

            self.image_path_label.setText(
                f"قالب HTML انتخاب‌شده: {file_path}"
            )

            self.image_preview.clear()
            self.image_preview.setText(
                "در حال ساخت پیش‌نمایش قالب HTML..."
            )

            self.status_label.setText(
                "وضعیت: در حال پردازش قالب HTML..."
            )
            ...
            try:
                ...
                preview_path = self.capture_html_render_frame(
                    file_path,
                    width,
                    height,
                )
                ...
                if pixmap.isNull():
                    raise RuntimeError(
                        "تصویر پیش‌نمایش HTML قابل باز شدن نیست."
                    )
                ...
                self.status_label.setText(
                    "وضعیت: قالب HTML انتخاب و آماده شد"
                )

            except Exception as error:
                self.image_preview.clear()
                self.image_preview.setText(
                    "ساخت پیش‌نمایش HTML ناموفق بود"
                )

                QMessageBox.critical(
                    self,
                    "خطا در پیش‌نمایش HTML",
                    "امکان ساخت پیش‌نمایش از قالب HTML "
                    "وجود ندارد.\n\n"
            ...
            QMessageBox.warning(
                self,
                "فرمت پشتیبانی نمی‌شود",
                "فقط فرمت‌های زیر قابل استفاده هستند:\n\n"
                "PNG, JPG, JPEG, WEBP, BMP, HTML, HTM",
            )
            return

        # به‌روزرسانی خلاصه پروژه در صورت وجود متد مربوطه
        for method_name in (
            "update_summary",
    ...
    def refresh_subtitle_editor_button(self):
        """Enable the editor button only when an SRT file exists."""
        button = getattr(
            self,
            "edit_subtitle_button",
            None,
        )
        ...
        subtitle_path = self.get_render_subtitle_path()
        available = (
        ...
        if available:
            ...
            button.setToolTip(
                "ابتدا فایل MP3 و زیرنویس SRT را تولید کنید."
            )

    def open_subtitle_editor(self, checked=False):
        """Open the preferred subtitle file in a modal editor."""
        subtitle_path = self.get_render_subtitle_path()
        ...
        if subtitle_path is None:
            ...
            QMessageBox.warning(
                self,
                "فایل زیرنویس موجود نیست",
                "ابتدا از بخش تنظیم صدا، فایل MP3 و "
                "زیرنویس SRT را تولید کنید.",
            )
            return
        ...
        if saved_path is not None:
            ...
            if (
                ...
                self.generated_srt_path = saved_path

        # The normal editor save may have created an edited
        # version next to the original file.
        preferred_path = self.get_render_subtitle_path()

        if preferred_path is not None:
            self.generated_srt_path = preferred_path

        self.refresh_subtitle_editor_button()

    def choose_render_audio(self):
        file_name, _ = QFileDialog.getOpenFileName(
            self,
            "انتخاب فایل صدای گوینده",
            str(PROJECT_ROOT / "input"),
            (
                "فایل‌های صوتی "
        ...
        self.render_audio_label.setText(
            f"صدای انتخاب‌شده: {audio_path.name}"
        )

    def get_render_audio_path(self) -> Path | None:
        if (
            self.imported_audio_path is not None
            and self.imported_audio_path.exists()
        ):
            return self.imported_audio_path

        if (
            self.generated_audio_path is not None
            and self.generated_audio_path.exists()
        ):
            return self.generated_audio_path

        default_audio = (
            OUTPUT_DIR / "narration-generated.mp3"
        )
    ...
    def get_render_subtitle_path(self) -> Path | None:
        ...
        if self.generated_srt_path is not None:
            generated_path = Path(self.generated_srt_path)
            ...
            if generated_path.stem.lower().endswith(".edited"):
                ...
                candidates.append(
                    generated_path.with_name(
                        f"{generated_path.stem}.edited.srt"
                    )
                )
                candidates.append(generated_path)

        candidates.extend([
            OUTPUT_DIR / "narration-generated.edited.srt",
            OUTPUT_DIR / "narration-generated.srt",
        ])
    ...
    def get_video_dimensions(self) -> tuple[int, int]:
        horizontal_radio = getattr(
            self,
            "horizontal_radio",
            None,
        )
        square_radio = getattr(
            self,
            "square_radio",
            None,
        )
        ...
        if (
            horizontal_radio is not None
            and horizontal_radio.isChecked()
        ):
            return 1920, 1080

        if (
            square_radio is not None
            and square_radio.isChecked()
        ):
            return 1080, 1080

        return 1080, 1920

    def get_image_render_mode(self) -> str:
        text = self.image_mode_combo.currentText().strip()

        if any(
            word in text
            for word in ("برش", "پر کردن", "Fill", "fill")
        ):
            return "fill"

        if any(
            word in text
            for word in ("کشیده", "کش دادن", "Stretch", "stretch")
        ):
            return "stretch"

        return "fit"

    def capture_html_render_frame(
        self,
        html_path: Path,
        width: int,
        height: int,
    ) -> Path:
        """Capture the visible HTML viewport for FFmpeg rendering."""
        ...
        if not html_path.exists():
            raise FileNotFoundError(
                f"فایل HTML پیدا نشد: {html_path}"
            )

        render_dir = TEMP_DIR / "html-render"
        render_dir.mkdir(parents=True, exist_ok=True)

        output_path = render_dir / (
            f"{html_path.stem}-{width}x{height}.png"
        )
        ...
        if not output_path.exists():
            raise RuntimeError(
                "Playwright تصویر HTML را ایجاد نکرد."
            )

        if output_path.stat().st_size < 100:
            raise RuntimeError(
                "تصویر تولیدشده از HTML خالی یا ناقص است."
            )

        return output_path

    def start_video_render(self, preview: bool = False):
        if (
            self.render_worker is not None
            and self.render_worker.isRunning()
        ):
            QMessageBox.information(
                self,
                "رندر در حال اجرا است",
                "لطفاً عملیات فعلی را تمام یا لغو کنید.",
            )
            return

        audio_path = self.get_render_audio_path()

        if audio_path is None:
            QMessageBox.warning(
                self,
                "فایل صوتی موجود نیست",
                "ابتدا صدا را در برنامه تولید کنید یا "
                "فایل صدای ضبط‌شده خودتان را انتخاب کنید.",
            )
            return
        ...
        if subtitle_path is None:
            QMessageBox.warning(
                self,
                "فایل زیرنویس موجود نیست",
                "فایل SRT متناظر با صدا پیدا نشد.\n\n"
                "ابتدا از بخش تولید صدا، گزینه «تولید کامل "
                "MP3 و زیرنویس SRT» را اجرا کنید.",
            )
            return
        ...
        if not subtitle_path.exists():
            QMessageBox.warning(
                self,
                "فایل زیرنویس پیدا نشد",
                f"مسیر زیرنویس معتبر نیست:\n{subtitle_path}",
            )
            return

        if subtitle_path.stat().st_size < 20:
            QMessageBox.warning(
                self,
                "فایل زیرنویس خالی است",
                f"فایل زیرنویس خالی یا ناقص است:\n"
                f"{subtitle_path}",
            )
            return
        ...
        if preview:
            output_path = (
                OUTPUT_DIR / "video-preview-15s.mp4"
            )
        else:
            selected_path, _ = QFileDialog.getSaveFileName(
                self,
                "ذخیره ویدئوی نهایی",
                str(OUTPUT_DIR / "final-video.mp4"),
                "ویدئوی MP4 (*.mp4)",
            )
        ...
        if (
            self.current_image_file is not None
            and self.current_image_file.exists()
        ):
            image_path = self.current_image_file
        elif (
            self.current_text_file is not None
            and self.current_text_file.exists()
            and self.current_text_file.suffix.lower()
            in {".html", ".htm"}
        ):
            self.status_label.setText(
                "وضعیت: در حال ساخت تصویر از فایل HTML..."
            )
            QApplication.processEvents()

            try:
                image_path = self.capture_html_render_frame(
                    self.current_text_file,
                    width,
                    height,
                )
            except Exception as error:
                QMessageBox.critical(
                    self,
                    "خطا در پردازش HTML",
                    "امکان ساخت تصویر از فایل HTML وجود ندارد."
                    "\n\n"
                    f"{type(error).__name__}: {error}",
                )
                return
        ...
        if image_path is None:
            QMessageBox.warning(
                self,
                "تصویر ویدئو موجود نیست",
                "برای ساخت ویدئوی تصویری یکی از این کارها را "
                "انجام دهید:\n\n"
                "۱. یک فایل HTML انتخاب کنید.\n"
                "۲. یا یک تصویر پس‌زمینه انتخاب کنید.",
            )
            return
        ...
        self.render_worker = RenderWorker(
            ...
            width=width,
            height=height,
            fps=self.render_fps_spin.value(),
            crf=self.render_crf_spin.value(),
            image_mode=self.get_image_render_mode(),
            subtitle_font="Tahoma",
            subtitle_size=(
                self.render_subtitle_size_spin.value()
            ),
            subtitle_margin=(
                self.render_subtitle_margin_spin.value()
            ),
            preview_seconds=15 if preview else None,
            parent=self,
        )

        self.render_worker.progress_changed.connect(
            self.on_render_progress
        )
        self.render_worker.result_ready.connect(
            self.on_render_result
        )
        self.render_worker.error_occurred.connect(
        ...
        self.progress_bar.setValue(1)
        self.status_label.setText(
            "وضعیت: آماده‌سازی رندر ویدئو..."
        )

        self.render_worker.start()

    def cancel_video_render(self):
        if (
            self.render_worker is not None
            and self.render_worker.isRunning()
        ):
            self.status_label.setText(
                "وضعیت: در حال لغو رندر..."
            )
            self.cancel_render_button.setEnabled(False)
    ...
    def on_render_result(
        ...
        self.progress_bar.setValue(100)
        self.status_label.setText(
            "وضعیت: ویدئو با موفقیت ساخته شد"
        )

        title = (
            "پیش‌نمایش آماده شد"
            if is_preview
            else "ویدئوی نهایی آماده شد"
        )

        QMessageBox.information(
            self,
            title,
            "فایل ویدئو با موفقیت ساخته شد:\n\n"
            f"{output_path}",
        )

        self.play_video_file(Path(output_path))

    def play_video_file(self, video_path: Path):
    ...
    def on_render_error(self, message: str):
        ...
        self.progress_bar.setValue(0)
        self.status_label.setText(
            "وضعیت: خطا در ساخت ویدئو"
        )

        QMessageBox.critical(
            self,
            "خطای ساخت ویدئو",
            "ساخت ویدئو انجام نشد.\n\n"
            f"{message}",
        )
